...
class article{
	...
	**/
	public function getlist($filter){
		...
		$list = DB::query('select * from `article` where topicid = '.$filter["id"].' and status = '.$filter["status"].' limit '.$pagenum.','.$filter["pagesize"]);
		$count = DB::query('select count(id) as num from `article` where topicid = '.$filter["id"].' and status = '.$filter["status"]);
		return array(
			"list"=>$list,
			"count"=>$count['0']['num'],
		);
