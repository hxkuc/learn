...
class user{
	...
	*/
	...
	public function changesetting($data){
		$res = DB::exec('update `user` set auto_setindex = '.$data['auto_setindex'].', auto_delindex = '.$data['auto_delindex'].',remarks = "'.$data['remarks'].'" ,diycolor = "'.$data['diycolor'].'",backgroundtype = '.$data['backgroundtype'].' where id = '.$data['uid']);
		return $res;
	}
